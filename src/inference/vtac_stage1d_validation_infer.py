...
def sh(x):
 ...
 return h.hexdigest()
def main():
 archive=ROOT/'02_data/raw/vtac-1.0'; out=ROOT/'04_analysis/runs/stage1d_v1'; out.mkdir(parents=True,exist_ok=True)
 split=pd.read_csv(archive/'benchmark_data_split.csv'); split['split']=split['split'].astype(str).str.lower().str.strip().replace({'val':'validation'}); lab=pd.read_csv(archive/'event_labels.csv'); z=split[split['split']=='validation'].merge(lab,on=['record','event'],validate='one_to_one').sort_values(['record','event']).reset_index(drop=True)
 if len(z)!=495 or z.record.nunique()!=226 or int(z.decision.sum())!=141: raise RuntimeError('validation_metadata_gate')
 ...
 for s in SEEDS:
  ...
  scores.append(pr)
 outdf=z[['record','event','decision']].rename(columns={'decision':'y_true'}).copy(); outdf.insert(0,'split','validation');
 for s,pr in zip(SEEDS,scores): outdf[f'p_seed_{s}']=pr
 outdf['score']=np.mean(np.vstack(scores),axis=0)
 if len(outdf)!=495 or outdf.record.nunique()!=226 or int(outdf.y_true.sum())!=141 or outdf.event.duplicated().any() or not np.isfinite(outdf.score).all(): raise RuntimeError('validation_scores_gate')
 scorepath=out/'validation_scores.csv'; outdf[['split','record','event','y_true','score']].to_csv(scorepath,index=False); outdf.to_csv(out/'validation_scores_with_seeds.csv',index=False); (out/'validation_scores.sha256').write_text(sh(scorepath)+'  validation_scores.csv\n',encoding='ascii')
 (out/'validation_inference_manifest.json').write_text(json.dumps({'rows':495,'records':226,'true_vt':141,'false_vt':354,'event_exclusions':0,'all_finite':True,'post_alarm_samples_used_max':0,'models':{str(s):sh(modeldir/f'fcn_realtime_seed_{s}.pt') for s in SEEDS},'preprocessing_hash':sh(ROOT/'03_code/preprocessing/vtac_stage1b_pipeline_v3.py'),'score_hash':sh(scorepath)},indent=2),encoding='ascii')
